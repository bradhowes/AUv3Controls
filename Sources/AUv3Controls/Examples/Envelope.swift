...
struct EnvelopeView: View {
  ...
  var body: some View {
    ...
    let delayStore = Store(initialState: KnobFeature.State(parameter: delayParam, config: config)) {
      KnobFeature()
    }
    ...
    let attackParam = AUParameterTree.createParameter(withIdentifier: "ATTACK", name: "Attack", address: 2, min: 0.0,
                                                      max: 100.0, unit: .generic, unitName: nil, flags: [],
                                                      valueStrings: nil, dependentParameters: nil)
    let attackStore = Store(initialState: KnobFeature.State(parameter: attackParam, config: config)) {
      KnobFeature()
    }
    ...
    let holdParam = AUParameterTree.createParameter(withIdentifier: "HOLD", name: "Hold", address: 3, min: 0.0,
                                                    max: 100.0, unit: .generic, unitName: nil, flags: [],
                                                    valueStrings: nil, dependentParameters: nil)
    let holdStore = Store(initialState: KnobFeature.State(parameter: holdParam, config: config)) {
      KnobFeature()
    }
    ...
    let decayParam = AUParameterTree.createParameter(withIdentifier: "DECAY", name: "Decay", address: 4, min: 0.0,
                                                     max: 100.0, unit: .generic, unitName: nil, flags: [],
                                                     valueStrings: nil, dependentParameters: nil)
    let decayStore = Store(initialState: KnobFeature.State(parameter: decayParam, config: config)) {
      KnobFeature()
    }
    ...
    let sustainParam = AUParameterTree.createParameter(withIdentifier: "SUSTAIN", name: "Sustain", address: 5, min: 0.0,
                                                       max: 100.0, unit: .generic, unitName: nil, flags: [],
                                                       valueStrings: nil, dependentParameters: nil)
    let sustainStore = Store(initialState: KnobFeature.State(parameter: sustainParam, config: config)) {
      KnobFeature()
    }
    ...
    let releaseParam = AUParameterTree.createParameter(withIdentifier: "RELEASE", name: "Release", address: 6, min: 0.0,
                                                       max: 100.0, unit: .generic, unitName: nil, flags: [],
                                                       valueStrings: nil, dependentParameters: nil)
    let releaseStore = Store(initialState: KnobFeature.State(parameter: releaseParam, config: config)) {
      KnobFeature()
    }
